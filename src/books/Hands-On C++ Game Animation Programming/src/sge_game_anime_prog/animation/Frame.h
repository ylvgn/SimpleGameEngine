...
namespace sge {
/*
	specialized frame and curve types, such as
		-a scalar frame,
		-a vector frame,
		-and a quaternion frame

	Similarly, you could create separate classes to represent
		-a scalar curve,
		-a vector curve,
		-and a quaternion curve
...
struct FrameUtil {
	...
	static QuaternionFrame	createFrame(float time, const quat4f& in, const quat4f& value, const quat4f& out);
...
inline
ScalarFrame FrameUtil::createFrame(float time, float in, float value, float out) {
	ScalarFrame res;
	res.time     = time;
	res.in[0]    = in;
	res.value[0] = value;
...
inline
VectorFrame FrameUtil::createFrame(float time, const vec3f& in, const vec3f& value, const vec3f& out) {
	VectorFrame res;
	res.time     = time;
	res.in[0]    = in.x;
	res.in[1]    = in.y;
...
inline
QuaternionFrame FrameUtil::createFrame(float time, const quat4f& in, const quat4f& value, const quat4f& out) {
	QuaternionFrame res;
	res.time     = time;
	res.in[0]    = in.x;
	res.in[1]    = in.y;
...
inline
ScalarFrame FrameUtil::createFrame(float time, float value) {
	return createFrame(time, 0, value, 0);
}

inline
VectorFrame FrameUtil::createFrame(float time, const vec3f& value) {
	return createFrame(time, vec3f::s_zero(), value, vec3f::s_zero());
}

inline
QuaternionFrame FrameUtil::createFrame(float time, const quat4f& value) {
