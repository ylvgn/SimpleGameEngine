...
LRESULT CALLBACK PW5_TextMetric::s_WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
	case WM_PAINT: {
			...
			*/
			...
			hdc.textOutf(300, 300, "cxChar={}, cyChar={}, cxCaps={}", cxChar, cyChar, cxCaps); // g_textOutf(hdc, 200, 200, L"cxChar=%i, cyChar=%i, cxCaps=%i", cxChar, cyChar, cxCaps);
	} break;
	} // switch
