#include "WavefrontObjLoader.h"

namespace sge {

void WavefrontObjLoader::readFile(EditMesh& outInfo, StrView filename) {
	MemMapFile mm;
	mm.open(filename);
	readMem(outInfo, mm, filename);
}

void WavefrontObjLoader::readMem(EditMesh& outInfo, ByteSpan data, StrView filename) {
	WavefrontObjLoader inst;
	inst._readMem(outInfo, data, filename);
}

void WavefrontObjLoader::_readMem(EditMesh& outInfo, ByteSpan data, StrView filename) {
	outInfo.clear();
	_outInfo = &outInfo;
	reset(data, filename);

	for (;;) {
		if (_token.isIdentifier("v"))	{ _parseLine_v();	continue; }
		if (_token.isIdentifier("vt"))	{ _parseLine_vt();	continue; }
		if (_token.isIdentifier("vn"))	{ _parseLine_vn();	continue; }
		if (_token.isIdentifier("f"))	{ _parseLine_f();	continue; }

		if (!nextToken()) break;
	}
}

void WavefrontObjLoader::_readVertexDataNumber(String& out) {
	out.clear();
	trimSpaces();
	auto* st = _cur - 1;
	int i = 0;
	while (_ch != ' ' && _ch != '\n') {
		i++;
		if (!nextChar()) break;
	}
	out.assign(st, st + i);
}

void WavefrontObjLoader::_parseLine_v() {
	Tuple4f v(0,0,0,1);
	_readVertexDataLineNumbers(v);
	_tmpPos.emplace_back(v.x / v.w, v.y / v.w, v.z / v.w);
}

void WavefrontObjLoader::_parseLine_vt() {
	auto& o = _tmpUv.emplace_back(0,0);
	_readVertexDataLineNumbers(o);
}

void WavefrontObjLoader::_parseLine_vn() {
	auto& o = _tmpNormal.emplace_back(0,0,0);
	_readVertexDataLineNumbers(o);
}

void WavefrontObjLoader::_parseLine_f() {
	Vector<int, 64>	face_v;
	Vector<int, 64>	face_vt;
	Vector<int, 64>	face_vn;
	Tuple3i _tmp = Tuple3i(0,0,0); // x:v, y:vt, z:vn

	const char* last = _cur;
	while (nextChar()) {
		if (_ch == ' ' || _ch == '\n' || _cur == _source.end()) {
			StrView faceItem = StrView(last, _cur - last - 1);
			StrView str = StringUtil::trimChar(faceItem, " \t\r");
			if (!str.empty()) {
//				SGE_LOG("now:{}", str);
				_tmp.x = 0;
				_tmp.y = 0;
				_tmp.z = 0;
				_parseLine_f_inner(_tmp, str, StringUtil::hasChar(str, '/'));

				int v  = _tmp.x;
				int vt = _tmp.y;
				int vn = _tmp.z;

//				SGE_LOG("\t\tv:{}, vt:{}, vn:{}, ch:{}", v, vt, vn, static_cast<int>(_ch));

				// index start from 1 and Negative values indicate relative vertex numbers
				if (v != 0) {
					if (v > 0) { v--; }
					else { v = static_cast<int>(_tmpPos.size()) + v; }
					face_v.emplace_back(v);
				}

				if (vt != 0) {
					if (vt > 0) { vt--; }
					else { vt = static_cast<int>(_tmpUv.size()) + vt; }
					face_vt.emplace_back(vt);
				}

				if (vn != 0) {
					if (vn > 0) { vn--; }
					else { vn = static_cast<int>(_tmpNormal.size()) + vn; }
					face_vn.emplace_back(vn);
				}
			}

			last = _cur;
		}

		if (_ch == '\n') {
			break;
		}
	}
	nextToken(); // skip '\n'

	if (face_v.size() < 3) {
		_error("face size < 3 (not a triangle face)");
	}

	u32 indicesStart = static_cast<u32>(_outInfo->pos.size());

#if 0 // flip face front/back
	eastl::reverse(face_v.begin(), face_v.end());
	eastl::reverse(face_vt.begin(), face_vt.end());
	eastl::reverse(face_vn.begin(), face_vn.end());
#endif

	for (int i = 0; i < face_v.size(); i++) {
		auto v = face_v[i];

		if (v >= _tmpPos.size()) _error("face v out of range");
		_outInfo->pos.emplace_back(_tmpPos[v]);

		if (face_vt.size() >= face_v.size()) {
			auto vt = face_vt[i];
			if (vt >= _tmpUv.size()) _error("face vt out of range");
			_outInfo->uv[0].emplace_back(_tmpUv[vt]);
		} else {
			_outInfo->uv[0].emplace_back(0,0); // fallback
		}

		if (face_vn.size() >= face_v.size()) {
			auto vn = face_vn[i];
			if (vn >= _tmpNormal.size()) _error("face vn out of range");
			_outInfo->normal.emplace_back(_tmpNormal[vn]);
		} else {
			_outInfo->normal.emplace_back(0,0,0); // fallback
		}
	}

	for (size_t i = 0; i < face_v.size(); i++) {
		if (i >= 3) {
			_outInfo->indices.emplace_back(static_cast<u32>(indicesStart + 0));
			_outInfo->indices.emplace_back(static_cast<u32>(indicesStart + i - 1));
		}

		u32 vi = static_cast<u32>(indicesStart + i);
		if (vi >= _outInfo->pos.size()) {
			throw SGE_ERROR("vi out of range");
		}
		_outInfo->indices.emplace_back(vi);
	}
}

void WavefrontObjLoader::_parseLine_f_inner(Tuple3i& out, StrView str, bool isSlash) {
	if (isSlash) { // v1/vt1/vn1 or v1//vn1
		for (int i = 0; i < 3; ++i) {
			auto s = StringUtil::splitByChar(str, '/');
			auto& first = s.first;
			if (first.size()) {
				if (!StringUtil::tryParse(first, out.data[i])) {
					_error("error _parseLine_f_inner with slash");
				}
			}
			str = s.second;
		}
	}
	else { // v1 v2 v3
		while (str.size()) {
			auto s = StringUtil::splitByChar(str, ' ');
			auto& first = s.first;
			if (first.size()) {
				if (!StringUtil::tryParse(first, out.data[0])) {
					_error("error _parseLine_f_inner without slash");
				}
			}
			str = s.second;
		}
	}
}

} // namespace